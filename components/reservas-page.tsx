            ...
            <div className="space-y-2">
              {(selectedBranch?.rooms || []).map((room) => {
                ...
                return <div key={room.id} className={`rounded-xl border p-3 transition-colors ${assignment ? "border-brand-orange bg-[#FFF9F5] shadow-[0_0_0_1px_rgba(255,90,31,0.12)]" : "border-brand-line bg-white"}`}>
                  <label className="flex cursor-pointer items-start gap-3 text-brand-ink"><input aria-label={`Incluir ${room.name}`} className="mt-0.5 h-5 w-5 shrink-0 cursor-pointer accent-[#FF5A1F]" type="checkbox" checked={Boolean(assignment)} onChange={(event) => setReservationForm((current) => ({ ...current, eventRooms: event.target.checked ? [...current.eventRooms, { roomId: room.id, allocatedCovers: "", usage: "partial" }] : current.eventRooms.filter((item) => item.roomId !== room.id) }))} /><span className="min-w-0 flex-1"><span className="block text-sm font-bold text-brand-ink">{assignment ? "Salón incluido:" : "Incluir salón:"} {room.name}</span><span className="mt-1 block text-xs leading-relaxed text-neutral-700">Capacidad nominal: {capacity} cubiertos. Es una referencia operativa y puede superarse con excepción autorizada.</span></span></label>
