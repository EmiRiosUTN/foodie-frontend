...
type ExceptionWindow = { service: "lunch" | "dinner"; startTime: string; endTime: string; intervalMin: number };
type Exception = { branchId: string; serviceDate: string; type: "closed" | "custom_hours" | "fully_booked" | "booking_disabled"; windows: ExceptionWindow[] };
...
export function OnlineBookingsPage({ mode = "public" }: { mode?: "rules" | "public" }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/restaurant/online-booking`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then((data) => {
        ...
        const incomingExceptions = Array.isArray(data.exceptions) ? data.exceptions.map((exception) => ({ ...exception, windows: Array.isArray(exception.windows) ? exception.windows : [] })) : [];
        ...
        setConfig({ ...data, bookingWindows: [...incomingWindows, ...defaults], exceptions: incomingExceptions });
      })
  ...
  const addException = (branchId: string) => setConfig((current) => current ? { ...current, exceptions: [...current.exceptions, { branchId, serviceDate: "", type: "closed", windows: [] }] } : current);
  const updateException = (index: number, patch: Partial<Exception>) => setConfig((current) => current ? { ...current, exceptions: current.exceptions.map((item, itemIndex) => itemIndex === index ? { ...item, ...patch } : item) } : current);
  const removeException = (index: number) => setConfig((current) => current ? { ...current, exceptions: current.exceptions.filter((_, itemIndex) => itemIndex !== index) } : current);
  const updateExceptionWindow = (index: number, service: ExceptionWindow["service"], patch: Partial<ExceptionWindow>) => setConfig((current) => current ? { ...current, exceptions: current.exceptions.map((item, itemIndex) => itemIndex !== index ? item : { ...item, windows: item.windows.map((window) => window.service === service ? { ...window, ...patch } : window) }) } : current);
  const toggleExceptionWindow = (index: number, service: ExceptionWindow["service"]) => setConfig((current) => current ? { ...current, exceptions: current.exceptions.map((item, itemIndex) => { if (itemIndex !== index) return item; const exists = item.windows.some((window) => window.service === service); return { ...item, type: "custom_hours", windows: exists ? item.windows.filter((window) => window.service !== service) : [...item.windows, { service, startTime: service === "lunch" ? "12:00" : "19:00", endTime: service === "lunch" ? "15:00" : "23:00", intervalMin: 30 }] }; }) } : current);
  ...
  return <WorkspaceShell title={title} description={description}><div className="space-y-6">
    {mode === "public" ? <PublicSettings config={config} publicUrl={publicUrl} update={updateSettings} /> : <><RulesSettings settings={config.settings} update={updateSettings} /><Schedules branches={config.restaurant.branches} bookingWindows={config.bookingWindows} updateWindow={updateWindow} updateDuration={updateDuration} /><Exceptions branches={config.restaurant.branches} exceptions={config.exceptions} add={addException} update={updateException} remove={removeException} toggleWindow={toggleExceptionWindow} updateWindow={updateExceptionWindow} /></>}
...
function Exceptions({ branches, exceptions, add, update, remove, toggleWindow, updateWindow }: { branches: Config["restaurant"]["branches"]; exceptions: Exception[]; add: (branchId: string) => void; update: (index: number, patch: Partial<Exception>) => void; remove: (index: number) => void; toggleWindow: (index: number, service: ExceptionWindow["service"]) => void; updateWindow: (index: number, service: ExceptionWindow["service"], patch: Partial<ExceptionWindow>) => void }) {
  return <section className="rounded-[26px] border border-brand-line bg-white p-5 md:p-7"><div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="font-extrabold text-brand-ink">Excepciones de calendario</h2><p className="mt-1 text-sm text-neutral-500">Bloqueá una fecha completa o configurá horarios especiales por turno.</p></div>{branches.map((branch) => <button key={branch.id} type="button" onClick={() => add(branch.id)} className="rounded-full border border-brand-orange px-4 py-2 text-sm font-bold text-brand-orange">Agregar fecha · {branch.name}</button>)}</div><div className="mt-5 space-y-4">{exceptions.length === 0 ? <p className="rounded-2xl bg-neutral-50 p-4 text-sm text-neutral-500">No hay excepciones configuradas.</p> : exceptions.map((exception, index) => <div key={`${exception.branchId}-${index}`} className="rounded-2xl border border-brand-line p-4"><div className="grid gap-3 md:grid-cols-[1fr_1fr_auto] md:items-end"><Field label="Sede"><select value={exception.branchId} onChange={(event) => update(index, { branchId: event.target.value })} className="foodie-input">{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></Field><Field label="Fecha"><input type="date" value={exception.serviceDate} onChange={(event) => update(index, { serviceDate: event.target.value })} className="foodie-input" /></Field><button type="button" onClick={() => remove(index)} className="rounded-full border border-red-200 px-4 py-3 text-sm font-bold text-red-600">Eliminar</button></div><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => update(index, { type: "closed", windows: [] })} className={`rounded-full px-4 py-2 text-sm font-bold ${exception.type === "closed" ? "bg-brand-orange text-white" : "bg-neutral-100 text-neutral-700"}`}>Cerrado todo el día</button><button type="button" onClick={() => update(index, { type: "custom_hours" })} className={`rounded-full px-4 py-2 text-sm font-bold ${exception.type === "custom_hours" ? "bg-brand-orange text-white" : "bg-neutral-100 text-neutral-700"}`}>Horario especial</button></div>{exception.type === "custom_hours" ? <div className="mt-4 grid gap-3 md:grid-cols-2">{(["lunch", "dinner"] as const).map((service) => { const window = exception.windows.find((item) => item.service === service); return <div key={service} className="rounded-xl border border-brand-line p-3"><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={Boolean(window)} onChange={() => toggleWindow(index, service)} />{service === "lunch" ? "Almuerzo" : "Cena"}</label>{window ? <div className="mt-3 grid grid-cols-3 gap-2"><input type="time" value={window.startTime} onChange={(event) => updateWindow(index, service, { startTime: event.target.value })} className="foodie-input py-2" /><input type="time" value={window.endTime} onChange={(event) => updateWindow(index, service, { endTime: event.target.value })} className="foodie-input py-2" /><input type="number" min="5" value={window.intervalMin} onChange={(event) => updateWindow(index, service, { intervalMin: Number(event.target.value) })} className="foodie-input py-2" /></div> : null}</div>; })}</div> : null}</div>)}</div></section>;
}
